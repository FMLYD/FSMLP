                ...
                if self.args.use_amp:
                    ...
                    if 'Linear' in self.args.model or 'TST' in self.args.model or 'PDF' in self.args.model:
                        ...
                        if self.args.output_attention:
                            ...
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                # print((time.time() - epoch_time));exit()
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                loss=loss+fre_loss(f_o,f_y)
                # if self.args.model=='CNN_Mixer':
                #     loss+=self.model.model.mix.loss()
                # if hasattr(self.model.model.mix,'random_emd'):
                #     # W=self.model.model.mix.random_emd.weight
                #     # loss+= torch.norm(W @ W.T - torch.eye(W.size(0), device=W.device), p='fro')
                #     for param in self.model.model.mix.random_emd.parameters():
            ...
            mean_val.append(vali_loss)
            mean_train.append(train_loss)
            mean_test.append(test_loss)
...
# 保存到文件
        torch.save(mean_dict, f'{self.args.model}.pt')
        return self.model
    ...
    def test(self, setting, test=0):
        ...
        performance_metrics = {
            'mae': mae,   # 这里替换成你的实际值
            'mse': mse,   # 这里替换成你的实际值
            # 'rmse': rmse,  # 这里替换成你的实际值
            # 'mape': mape,  # 这里替换成你的实际值
            # 'mspe': mspe,  # 这里替换成你的实际值
            # 'rse': rse,   # 这里替换成你的实际值
        }
        for key in performance_metrics:
            if isinstance(performance_metrics[key], np.ndarray):
                performance_metrics[key] = performance_metrics[key].tolist()
            else:
                performance_metrics[key] = float(performance_metrics[key]) 
        # 构建保存路径
